...
| Макс. удалений за интервал (0 = без лимита) | `--max-deletions-per-interval` | `MAX_DELETIONS_PER_INTERVAL` | `50` |
...
Лимит удалений считается в окне длиной `sync-period-seconds`: при отказе целой зоны
оператор добивает поды волнами (по 50 за окно по умолчанию), не создавая лавину
запросов к API-серверу; «лишние» поды автоматически переносятся на следующее окно.
Период опроса — это полный ресинк watch-кэша, страховка от пропущенных событий.
...
| `config.maxDeletionsPerInterval` | `50` | макс. удалений за интервал (0 = без лимита) |
| `config.syncPeriodSeconds` | `600` | период опроса/ресинка кластера, сек |
